...
public class PlatForm : NetworkBehaviour
{
    ...
    [SerializeField] private float startYPos; // 시작 Y 위치
    [SerializeField] private float targetYPos; // 목표 Y 위치
    ...
    [SerializeField] private bool isActive; // 목표 인원 도달 여부
    ...
    private void FixedUpdate()
    {
        ...
        bool desiredIsActive = GetTotalCeilCnt() >= targetMoveCnt;

        if (!isLocked && desiredIsActive != pendingIsActive)
        {
            pendingIsActive = desiredIsActive;
            StartLock();
            return;
        }

        if (!isLocked && isActive != pendingIsActive)
        {
    ...
    {
        float targetY = isActive ? targetYPos : startYPos;
