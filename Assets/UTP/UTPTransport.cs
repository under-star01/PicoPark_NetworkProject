...
namespace Mirror.Transports.Utp
{
    public class UTPTransport : Transport
    {
        ...
        {
            if (m_ServerConns.TryGetValue(connectionId, out var conn) && m_Driver.GetConnectionState(conn)
                == Unity.Networking.Transport.NetworkConnection.State.Connected)
            {
        ...
        private void Tick()
        {
            ...
            if (NetworkServer.active)
            {
                ...
                while ((incoming = m_Driver.Accept()) != default) 
                {
            ...
            while ((evt = m_Driver.PopEvent(out Unity.Networking.Transport.NetworkConnection conn,
                out DataStreamReader stream)) != NetworkEvent.Type.Empty)
            {
                switch (evt)
                {
                    ...
                    case NetworkEvent.Type.Data:
                        ...
                        if (NetworkServer.active) OnServerDataReceived?.Invoke(conn.GetHashCode(),
                            new ArraySegment<byte>(data), 0);
